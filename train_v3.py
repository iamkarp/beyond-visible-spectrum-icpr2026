"""
Beyond Visible Spectrum v3 - Targeted improvements to reach 0.78+

Key additions over v2:
1. Spectral library matching (Mahalanobis distance to class prototypes)
2. Continuum removal features from HS data
3. Per-pixel spectral classification with aggregation
4. Feature selection (mutual information + correlation filtering)
5. Pseudo-labeling with iterative refinement
6. Spectral angle mapper features
"""
...
# ============================================================
# NEW: Continuum Removal
# ============================================================
def continuum_removal(spectrum):
    """
    Apply continuum removal to a spectrum.
    This normalizes the spectrum to highlight absorption features.
    """
    n = len(spectrum)
    if n < 3:
        return spectrum
    # Build convex hull (upper envelope)
    hull = spectrum.copy()
    # Simple iterative upper hull
    indices = list(range(n))
    hull_points = [0, n - 1]  # Start and end
    ...
    while changed:
        ...
        for i in range(1, len(hull_points)):
            ...
            for j in range(start + 1, end):
                # Interpolated value on the line from start to end
                t = (j - start) / (end - start)
    ...
    # Interpolate hull
    hull_interp = np.interp(indices, hull_points, spectrum[hull_points])
    # Continuum removed = spectrum / hull
    cr = spectrum / (hull_interp + 1e-8)
    return cr


# ============================================================
# NEW: Spectral Angle Mapper
# ============================================================
...
def extract_ms_features(path):
    ...
    def safe_idx(name, num, den):
        idx_arr = num / (den + eps)
        features[f"ms_{name}_mean"] = np.mean(idx_arr)
        features[f"ms_{name}_std"] = np.std(idx_arr)
        features[f"ms_{name}_min"] = np.min(idx_arr)
        features[f"ms_{name}_max"] = np.max(idx_arr)
        features[f"ms_{name}_median"] = np.median(idx_arr)
        features[f"ms_{name}_q25"] = np.percentile(idx_arr, 25)
        features[f"ms_{name}_q75"] = np.percentile(idx_arr, 75)
        features[f"ms_{name}_range"] = np.max(idx_arr) - np.min(idx_arr)
    ...
    # Pixel-level NDVI distribution features
    ndvi_flat = ndvi.ravel()
    features["ms_ndvi_positive_frac"] = np.mean(ndvi_flat > 0)
    features["ms_ndvi_high_frac"] = np.mean(ndvi_flat > 0.5)
    features["ms_ndvi_low_frac"] = np.mean(ndvi_flat < 0.2)
...
def extract_hs_features(path, drop_first=10, drop_last=14, n_pca=25):
    ...
    # === NEW: Continuum removal features ===
    ...
    # Absorption depth at key positions
    features["hs_cr_depth_red"] = 1.0 - cr_spectrum[int(C * 0.40)]  # Red absorption
    features["hs_cr_depth_rededge"] = 1.0 - cr_spectrum[int(C * 0.55)]  # Red edge
    features["hs_cr_depth_green"] = 1.0 - cr_spectrum[int(C * 0.15)]  # Green
    # Width of absorption features
    absorption_mask = cr_spectrum < 0.95
    features["hs_cr_absorption_width"] = np.sum(absorption_mask) / C

    # Continuum removal of median spectrum too
    ...
    # === NEW: Pixel spectral heterogeneity ===
    # How varied are pixel spectra within this image?
    pixel_stds = np.std(flat, axis=0)  # std across pixels for each band
    features["hs_pixel_hetero_mean"] = np.mean(pixel_stds)
    features["hs_pixel_hetero_std"] = np.std(pixel_stds)
    features["hs_pixel_hetero_max"] = np.max(pixel_stds)
    ...
    return features, mean_spectrum, flat
...
def extract_all_features(row):
    ...
    if pd.notna(row.get("hs")):
        hs_feats, hs_mean_spectrum, hs_flat = extract_hs_features(row["hs"])
        features.update(hs_feats)
    if pd.notna(row.get("rgb")) and pd.notna(row.get("ms")):
        features.update(extract_cross_modal_features(row["rgb"], row["ms"]))

    return features, hs_mean_spectrum


# ============================================================
# Spectral Library Matching Features
# ============================================================
def compute_class_spectral_libraries(train_df, spectra_dict):
    """Compute per-class spectral statistics for library matching."""
    # Find target length (most common)
    lengths = [len(s) for s in spectra_dict.values() if s is not None]
    target_len = min(lengths) if lengths else 101
    ...
    for label in LABELS:
        ...
        for _, row in class_df.iterrows():
            bid = row["base_id"]
            if bid in spectra_dict and spectra_dict[bid] is not None:
                spec = spectra_dict[bid][:target_len]  # Truncate to consistent length
                class_spectra.append(spec)
        if class_spectra:
            class_spectra = np.array(class_spectra)
            libraries[label] = {
                "mean": np.mean(class_spectra, axis=0),
                "std": np.std(class_spectra, axis=0) + 1e-8,
                "cov_inv": None,  # Will compute if needed
            }
            try:
                cov = np.cov(class_spectra.T) + np.eye(class_spectra.shape[1]) * 1e-4
                libraries[label]["cov_inv"] = np.linalg.inv(cov)
            except:
                pass
    return libraries


def spectral_library_features(spectrum, libraries, prefix="slib"):
    """Compute distance features from a spectrum to class prototypes."""
    features = {}
    if spectrum is None:
        return features

    for label, lib in libraries.items():
        # Truncate spectrum to match library length
        target_len = len(lib["mean"])
        spec = spectrum[:target_len]
        # Euclidean distance to class mean
        diff = spec - lib["mean"]
        features[f"{prefix}_{label}_euclid"] = np.sqrt(np.sum(diff**2))

        # Normalized Euclidean distance (per-band)
        features[f"{prefix}_{label}_norm_euclid"] = np.sqrt(np.sum((diff / lib["std"])**2))

        # Spectral angle
        features[f"{prefix}_{label}_angle"] = spectral_angle(spec, lib["mean"])

        # Correlation
        features[f"{prefix}_{label}_corr"] = np.corrcoef(spec, lib["mean"])[0, 1]

        # Mahalanobis distance (if covariance available)
        if lib["cov_inv"] is not None:
            try:
                maha = np.sqrt(diff @ lib["cov_inv"] @ diff)
                features[f"{prefix}_{label}_mahalanobis"] = maha
            except:
                pass

    # Relative distances (ratios between class distances)
    for l1 in LABELS:
        for l2 in LABELS:
            if l1 < l2:
                key1 = f"{prefix}_{l1}_euclid"
                key2 = f"{prefix}_{l2}_euclid"
                if key1 in features and key2 in features:
                    features[f"{prefix}_{l1}_{l2}_ratio"] = features[key1] / (features[key2] + 1e-8)
...
def main():
    print("=" * 60)
    print("Beyond Visible Spectrum v3")
    print("=" * 60)
    ...
    for i, (_, row) in enumerate(train_df.iterrows()):
        feats, spectrum = extract_all_features(row)
        train_features.append(feats)
        train_spectra[row["base_id"]] = spectrum
        if (i + 1) % 100 == 0:
    ...
    for i, (_, row) in enumerate(val_df.iterrows()):
        ...
        val_spectra[row["base_id"]] = spectrum
        if (i + 1) % 100 == 0:
            print(f"  Val: {i + 1}/{len(val_df)}")

    # Phase 2: Compute spectral library matching features
    print("\nPhase 2: Spectral library matching...")
    libraries = compute_class_spectral_libraries(train_df, train_spectra)
    print(f"  Library classes: {list(libraries.keys())}")

    for i, row in enumerate(train_features):
        bid = row["base_id"]
        lib_feats = spectral_library_features(train_spectra.get(bid), libraries)
        row.update(lib_feats)

    for i, row in enumerate(val_features):
        bid = row["base_id"]
        lib_feats = spectral_library_features(val_spectra.get(bid), libraries)
        row.update(lib_feats)

    train_feat_df = pd.DataFrame(train_features)
    val_feat_df = pd.DataFrame(val_features)
    print(f"  Total features: {train_feat_df.shape[1] - 1}")
    ...
    # Also compute correlation-based redundancy
    top_k = min(500, len(feature_cols))
    selected_idx = mi_order[:top_k].tolist()
    print(f"  Selected top {len(selected_idx)} features by MI")

    # Remove highly correlated features
    X_sel = X_train[:, selected_idx]
    corr_matrix = np.abs(np.corrcoef(X_sel.T))
    to_drop = set()
    ...
    final_idx = [selected_idx[i] for i in range(len(selected_idx)) if i not in to_drop]
    print(f"  After correlation filtering: {len(final_idx)} features")

    X_train_sel = X_train[:, final_idx]
    X_test_sel = X_test[:, final_idx]
    sel_feature_names = [feature_cols[i] for i in final_idx]

    # Also keep full feature set for diversity
    print(f"\n  Full features: {X_train.shape[1]}")
    ...
    # Model configs with different feature sets
    model_configs = [
        # LightGBM on selected features
        ("lgb_sel_v1", "sel", {
            "objective": "multiclass", "num_class": 3, "metric": "multi_logloss",
            "learning_rate": 0.02, "num_leaves": 25, "max_depth": 5,
        ...
        ("lgb_sel_v2", "sel", {
            "objective": "multiclass", "num_class": 3, "metric": "multi_logloss",
            "learning_rate": 0.015, "num_leaves": 15, "max_depth": 4,
            "min_child_samples": 15, "subsample": 0.8, "colsample_bytree": 0.6,
            "reg_alpha": 1.0, "reg_lambda": 1.5, "n_estimators": 4000,
        ...
        ("lgb_full_v1", "full", {
            ...
            "min_child_samples": 8, "subsample": 0.75, "colsample_bytree": 0.5,
        ...
        ("lgb_full_v2", "full", {
            ...
            "min_child_samples": 12, "subsample": 0.8, "colsample_bytree": 0.4,
            "reg_alpha": 1.0, "reg_lambda": 1.0, "n_estimators": 3000,
            "verbose": -1, "random_state": SEED + 5,
        }),
    ]

    for name, feat_type, params in model_configs:
        X_tr = X_train_sel if feat_type == "sel" else X_train
        X_te = X_test_sel if feat_type == "sel" else X_test
        ...
        for fold, (tr_idx, va_idx) in enumerate(skf.split(X_tr, y_train)):
            ...
            model.fit(
                ...
                eval_set=[(X_tr[va_idx], y_train[va_idx])],
                callbacks=[lgb.early_stopping(100, verbose=False)],
            )
            oof_probs[va_idx] = model.predict_proba(X_tr[va_idx])
    ...
    xgb_configs = [
        ("xgb_sel_v1", "sel", {
            ...
            "learning_rate": 0.02, "max_depth": 5, "min_child_weight": 5,
            "subsample": 0.75, "colsample_bytree": 0.7, "reg_alpha": 0.5,
            "reg_lambda": 1.0, "n_estimators": 3000, "tree_method": "hist",
            "random_state": SEED, "verbosity": 0,
        }),
        ("xgb_full_v1", "full", {
            "objective": "multi:softprob", "num_class": 3, "eval_metric": "mlogloss",
            "learning_rate": 0.03, "max_depth": 4, "min_child_weight": 8,
            "subsample": 0.75, "colsample_bytree": 0.5, "reg_alpha": 1.0,
            "reg_lambda": 2.0, "n_estimators": 2000, "tree_method": "hist",
            "random_state": SEED, "verbosity": 0,
        }),
    ]

    for name, feat_type, params in xgb_configs:
        X_tr = X_train_sel if feat_type == "sel" else X_train
        X_te = X_test_sel if feat_type == "sel" else X_test
        ...
        for fold, (tr_idx, va_idx) in enumerate(skf.split(X_tr, y_train)):
            ...
            model.fit(X_tr[tr_idx], y_train[tr_idx],
                      eval_set=[(X_tr[va_idx], y_train[va_idx])], verbose=False)
            oof_probs[va_idx] = model.predict_proba(X_tr[va_idx])
            test_probs += model.predict_proba(X_te) / N_FOLDS
    ...
    # sklearn models on selected features
    print("\n  Training sklearn models...")

    # Gradient Boosting
    oof_probs = np.zeros((len(X_train_sel), 3))
    test_probs = np.zeros((len(X_test_sel), 3))
    for fold, (tr_idx, va_idx) in enumerate(skf.split(X_train_sel, y_train)):
        ...
        oof_probs[va_idx] = model.predict_proba(X_train_sel[va_idx])
        test_probs += model.predict_proba(X_test_sel) / N_FOLDS
    f1 = f1_score(y_train, oof_probs.argmax(1), average="macro")
    acc = accuracy_score(y_train, oof_probs.argmax(1))
    print(f"  sklearn_gb: F1={f1:.4f} Acc={acc:.4f}")
    ...
    test_probs = np.zeros((len(X_test_sel), 3))
    for fold, (tr_idx, va_idx) in enumerate(skf.split(X_train_sel, y_train)):
        model = ExtraTreesClassifier(n_estimators=2000, max_depth=None, min_samples_leaf=2,
    ...
    for k in [3, 5, 7, len(sorted_models)]:
        ...
        weights = weights ** 3  # Cube weights to strongly emphasize better models
    ...
    # Phase 6: Pseudo-labeling
    print("\n" + "=" * 60)
    print("Phase 6: Pseudo-labeling")
    print("=" * 60)

    test_preds_conf = best_test.max(axis=1)
    threshold = 0.85
    pseudo_mask = test_preds_conf >= threshold
    n_pseudo = pseudo_mask.sum()
    print(f"  Confidence threshold: {threshold}")
    print(f"  Pseudo-labeled samples: {n_pseudo}/{len(X_test)} ({n_pseudo/len(X_test)*100:.1f}%)")

    if n_pseudo > 20:
        pseudo_y = best_test[pseudo_mask].argmax(1)
        X_train_aug = np.concatenate([X_train_sel, X_test_sel[pseudo_mask]], axis=0)
        y_train_aug = np.concatenate([y_train, pseudo_y], axis=0)
        print(f"  Augmented training set: {len(X_train_aug)} samples")

        # Retrain best models on augmented data
        oof_probs_pl = np.zeros((len(X_train_sel), 3))
        test_probs_pl = np.zeros((len(X_test_sel), 3))

        for fold, (tr_idx, va_idx) in enumerate(skf.split(X_train_sel, y_train)):
            # Add all pseudo-labels to training (they're never in validation)
            n_orig = len(tr_idx)
            X_fold_tr = np.concatenate([X_train_sel[tr_idx], X_test_sel[pseudo_mask]], axis=0)
            y_fold_tr = np.concatenate([y_train[tr_idx], pseudo_y], axis=0)
            ...
            model.fit(
                X_fold_tr, y_fold_tr,
                eval_set=[(X_train_sel[va_idx], y_train[va_idx])],
                callbacks=[lgb.early_stopping(100, verbose=False)],
            )
            oof_probs_pl[va_idx] = model.predict_proba(X_train_sel[va_idx])
            test_probs_pl += model.predict_proba(X_test_sel) / N_FOLDS

        pl_acc = accuracy_score(y_train, oof_probs_pl.argmax(1))
        pl_f1 = f1_score(y_train, oof_probs_pl.argmax(1), average="macro")
        print(f"  Pseudo-labeled LGB: Acc={pl_acc:.4f} F1={pl_f1:.4f}")

        # Use pseudo-labeled predictions if better
        if pl_acc > final_acc:
            print(f"  Pseudo-labeling improved! {final_acc:.4f} -> {pl_acc:.4f}")
            # Blend with original ensemble
            blend_test = 0.6 * best_test + 0.4 * test_probs_pl
            blend_acc = accuracy_score(y_train, (0.6 * best_oof + 0.4 * oof_probs_pl).argmax(1))
            print(f"  Blended Acc: {blend_acc:.4f}")

            if pl_acc > blend_acc:
                final_test = test_probs_pl
                final_acc = pl_acc
                final_f1 = pl_f1
            else:
                final_test = blend_test
                final_acc = blend_acc
                final_f1 = f1_score(y_train, (0.6 * best_oof + 0.4 * oof_probs_pl).argmax(1), average="macro")
        else:
            print(f"  No improvement from pseudo-labeling")
            final_test = best_test
    else:
        final_test = best_test

    # Save submission
    test_preds = final_test.argmax(1)
    sub_ids = []
    ...
    sub_name = f"submission_v3_oof_acc_{acc_str}_f1_{f1_str}.csv"
    sub_path = os.path.join(OUT_DIR, sub_name)
    sub.to_csv(sub_path, index=False)
    print(f"\nSubmission saved: {sub_path}")

    # Save probabilities for potential further stacking
    np.save(os.path.join(OUT_DIR, "v3_oof_probs.npy"), best_oof)
    np.save(os.path.join(OUT_DIR, "v3_test_probs.npy"), final_test)

    # Print top features
    print("\nTop 30 features by MI score:")
    for i in range(min(30, len(mi_order))):
        idx = mi_order[i]
        print(f"  {feature_cols[idx]:50s} MI={mi_scores[idx]:.4f}")
